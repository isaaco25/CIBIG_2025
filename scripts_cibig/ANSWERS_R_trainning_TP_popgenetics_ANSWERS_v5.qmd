...
put it into the object algeaInfo
:::
...
algeaInfo <- read.csv("input/coding_data_CIBIG25/metadata_algae.tsv", sep="\t", header=TRUE, check.names=FALSE)
algeaInfo <- read.csv(paste0(dataDir,"/coding_data_CIBIG25/metadata_algae.tsv"), sep="\t", header=TRUE, check.names=FALSE)
...
# Take a look at the files
list.files(dataDir, recursive = TRUE)
vcfData <- read.vcfR(paste0(dataDir, "/coding_data_CIBIG25/all_samples.vcf.gz"), verbose = FALSE)
```
...
plot(pcaScores$PC1, pcaScores$PC2)


```

In the pcaScores object the strain id are stored in the rownames, that are accessible using the rownames() function.
...
pcaScores$strainId %in% algeaInfo$strainId
```
...
algeaInfo$strainId <- gsub("RCC", "", algeaInfo$strainId)
...
#| code-fold: true
#| code-summary: "💭 **Check the answer** 🪂"
pcaScore <- left_join(x = pcaScores,
                       y = algeaInfo,
...
rownames(algaeInfo) <- algaeInfo$strainId

# reorder algeaInfo
algaeInfo <- algaeInfo[tips, ]

# compare the ids to check they are in the same order
algaeInfo$strainId
tips
...
Use `paste0()` to concatenate the character chains, as before to create `tip_species` and `tip_localisation`
...
# We use the "species" column from the metadata table
algeaInfo$Species

# and the localization 
algeaInfo$Sampling_site

tip_species <- algeaInfo$Species
tip_localisation <- algeaInfo$Sampling_site
...
Let's put this into a image file like before :

```{r}
jpeg(filename = paste0(outDir, "/tree_plot.jpg"),width = 750, height = 750)
print({
  plot(rooted_tree, cex = 0.9,show.tip.label = T, label.offset=25)
  tiplabels(pch = symbols, col = "black", bg = "white", cex = 1.2)
})
...
svg(filename = paste0(outDir, "/tree_plot.svg"),width = 7, height = 7)
print({
  plot(rooted_tree, cex = 0.9,show.tip.label = T, label.offset=25)
  tiplabels(pch = symbols, col = "black", bg = "white", cex = 1.2)
})
...
#| code-summary: "💭 **Check the answer** 🪂"
colnames(flagstats)
colnames(flagstats) <- c("passed", "failed", "category", "strainId")
colnames(flagstats)
head(flagstats)
```
...
unique(flagstats$category)

# what does the below do ?
table(flagstats$category)
```
...
flagstats <- flagstats
# flagstats$category <- as.factor(flagstats$category)
# levels(flagstats$category)
head(flagstats)
```
...
flagstats$strainId <- gsub(pattern = "_flagstat.tsv", replacement = "", flagstats$strainId)
...
flagstats[is.na(flagstats$failed),]
flagstats[is.na(flagstats$passed),]
```
...
flagstats[flagstats$strainId =="B218",]
```
...
head(flagstats$category)

grepl("%", flagstats$category)

statsRatios <- flagstats[grepl("%", flagstats$category),]
```
